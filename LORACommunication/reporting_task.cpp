...
void reporting_task(void){
	...
	while (tries < START_COMM_MAX_RETRIES) {
		...
		code = comm_start_report(available, 2); // 2 == post data
...
uint8_t reporting_test(uint8_t *buffer, int length) {
	...
	while (tries < START_COMM_MAX_RETRIES) {
		db("attempting to start report");
		code = comm_start_report(length, 1); // 1 = test
	...
	for (int i = 0; i < 50; i++) {
		Serial.print( (char) reply[i]);
	}
	Serial.println();
#endif // DEBUG

	// Check if server wants module to retest or report
	int report[6] = { 0x72, 0x65, 0x70, 0x6f, 0x72, 0x74 };
	for (int i = 0; i < 6; i++) {
		if (report[i] != reply[i])
			return RETEST;
	}
